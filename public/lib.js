...
class Word extends GameObject
  {
    ...
    {
      if(!ctx || !this.sprite) {return;}
      let cloud_word = this.matched == true? this.translation:this.word;
...
class Game 
  {
    ...
    SpawnWords()
    {
         if(this.game_state == "pause") {return;}
          ...
          if(!word) {return;}
          let word_object = new Word(word[0],word[1]);
    ...
    CheckForMatch()
    {
      ...
      for(var w in this.gameObjects)
        {
          if(this.gameObjects[w].word && this.gameObjects[w].translation)
            {
              if(this.gameObjects[w].destroyed || this.gameObjects[w].matched){continue;}
              if(this.input == this.gameObjects[w].translation)
                {
                  this.input = '';
            ...
            if(position <= 0)
              {
                ...
                if(this.gameObjects[wo].matched != true) 
                {
                  ...
                  if(last_missed_word){last_missed_word.innerHTML = this.gameObjects[wo].translation;}
